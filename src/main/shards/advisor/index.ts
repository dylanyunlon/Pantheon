...
export class AdvisorSettings {
  ...
  minPriority: PantheonAdvicePriority = PantheonAdvicePriority.INFO
...
export class CoachAdvisorMain implements IAkariShardInitDispose {
  ...
  private _engine: PantheonEngine
  private _dataTracker: PantheonDataTracker
  private _privacyPipeline: PrivacyPipeline | null = null

  public readonly settings = new AdvisorSettings()
  ...
  constructor(
    ...
    this._engine = createPantheonEngine()
    this._dataTracker = new PantheonDataTracker()
  ...
  private _generateAdvices() {
    try {
      ...
      if (!this.settings.showEnemyWeakness)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.ENEMY_WEAKNESS)
      if (!this.settings.showTeamSynergy)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.TEAM_SYNERGY)
      if (!this.settings.showMacroStrategy)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.MACRO_STRATEGY)
      if (!this.settings.showMentalAdvice)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.MENTAL)
      if (!this.settings.showLaneMatchup)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.LANE_MATCHUP)
      if (!this.settings.showRankDisparity)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.RANK_DISPARITY)
      if (!this.settings.showComposition)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.COMPOSITION)
      if (!this.settings.showItemization)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.ITEMIZATION_HINT)
      if (!this.settings.showObjectiveTiming)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.OBJECTIVE_TIMING)
      if (!this.settings.showPlaystyleAdaptation)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.PLAYSTYLE_ADAPTATION)
      if (!this.settings.showGoldEfficiency)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.GOLD_EFFICIENCY)
      if (!this.settings.showTrueDamageWarning)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.TRUE_DAMAGE_WARNING)
      if (!this.settings.showWinCondition)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.WIN_CONDITION)
      if (!this.settings.showKdaTrend)
        filtered = filtered.filter((a) => a.type !== PantheonAdviceType.KDA_TREND)
