    ...
    >
      <template #header>
        <div class="coach-header">
          <span class="coach-title">\U0001F3AF {{ t('AdvisorPanel.title') }}</span>
          <div class="coach-actions">
            <NPopover :delay="50" :keep-alive-on-hover="false">
              ...
              <div class="popover-text">
                {{ t('AdvisorPanel.pipelineTooltip') }}
              </div>
            </NPopover>
            ...
            <NPopover :delay="50" v-if="cas.state.teamComparisonSummary">
              ...
              <div class="dimension-detail">
                <div class="dimension-title">{{ t('AdvisorPanel.dimensionDetail') }}</div>
                <div
      ...
      <template v-if="!collapsed">
              ...
              <div v-if="!collapsedGroups.has(group.type)" class="group-items">
                ...
                >
                  ...
                  <div class="advice-item-actions">
                    <span class="advice-confidence" :title="t('AdvisorPanel.confidence')">{{ (advice.confidence * 100).toFixed(0) }}%</span>
        ...
        <div v-else-if="cas.state.isGenerating" class="coach-advice-list">
          <div class="advice-line analyzing">{{ t('AdvisorPanel.analyzing') }}</div>
        </div>
        <div v-if="failedPuuids.length > 0" class="failed-notice">
          <span>{{ t('AdvisorPanel.dataLoadFailed', { count: failedPuuids.length }) }}</span>
          <NButton size="tiny" quaternary @click="handleRefresh">{{ t('AdvisorPanel.retry') }}</NButton>
        </div>
      </template>
      <template #footer v-if="cas.state.lastGeneratedAt">
        <div class="coach-footer">
          <span class="coach-timestamp">
            {{ t('AdvisorPanel.generatedAt', { time: formattedTime }) }}
...
const schedulerStats = ref<Record<string, number | string> | null>(null)
...
const displayMessages = computed(() => {
  if (cas.state.isGenerating && cas.state.formattedMessages.length === 0) {
    return [t('AdvisorPanel.analyzing')]
  }
...
const scoreDiffLabel = computed(() => {
  ...
  if (!info) return ''
  if (info.scoreDiff > 3) return t('AdvisorPanel.teamAdvantage')
  if (info.scoreDiff < -3) return t('AdvisorPanel.teamDisadvantage')
  return t('AdvisorPanel.teamBalanced')
})
