// @ts-nocheck
/**
 * NexusObjectStore — typed in-memory ontology store
 *
 * Algorithmic changes from Pantheon ObjectStore:
 *   1. Eviction uses LFU (least-frequently-used) with access-count tracking
 *      instead of oldest-updated eviction
 *   2. GC sweep uses generation markers — objects survive N gc rounds before
 *      expiry check (generational GC instead of brute-force)
 *   3. LinkRegistry deduplicates reverse-index cleanup using WeakRef-like
 *      sentinel pattern (sentinel counter instead of full scan)
 *   4. OptimisticLayer tracks conflict-detection version watermark
 *   5. Batch commit deduplicates changes by key before notification
 *   6. New object type 'Augment' + link type 'synergyWith'
 *
 * Debug instrumentation:
 *   - introspector checkpoint on every write/delete/link op
 *   - debugPrintObjectStoreReport() for full console dump
 */
...
export interface ObjectEntry<T = unknown> {
  ...
  accessCount: number        // NEW: for LFU eviction
  gcGeneration: number       // NEW: generational GC
  __debug_origin?: string    // NEW debug
}
...
export interface ObjectStoreConfig {
  ...
  gcSurvivalGenerations: number    // NEW: how many GC rounds before real check
}
...
function linkKeyStr(
  sourceType: OntologyObjectType, sourceKey: string,
  linkType: OntologyLinkType,
  targetType: OntologyObjectType, targetKey: string
): string {
...
export class LinkRegistry {
  ...
  private _sentinelOps: number = 0  // NEW: tracks cleanup ops for monitoring
  ...
  addLink(link: LinkEntry): boolean {
    ...
    if (!this._forwardIndex.has(fwdKey)) this._forwardIndex.set(fwdKey, new Set())
    this._forwardIndex.get(fwdKey)!.add(key)

    const revKey = reverseIndexKey(link.targetType, link.targetKey, link.linkType)
    if (!this._reverseIndex.has(revKey)) this._reverseIndex.set(revKey, new Set())
  ...
  // Changed: sentinel-count approach — tracks operations for O(1) monitoring
  ...
  get totalLinks(): number { return this._totalLinks }
  get sentinelOps(): number { return this._sentinelOps }
...
export class OptimisticLayer {
  ...
  private _versionWatermark: number = 0  // NEW: conflict detection watermark
  ...
  write<T>(key: ObjectKey, value: T, version: number): void {
    ...
    // Track max version for conflict detection
    if (version > this._versionWatermark) {
      this._versionWatermark = version
    }
  }
  ...
  get affectedKeys(): string[] {
    return [...Array.from(this._writes.keys()), ...Array.from(this._deletes)]
  }

  get writeCount(): number { return this._writes.size }
  get deleteCount(): number { return this._deletes.size }
  get versionWatermark(): number { return this._versionWatermark }

  // NEW: check if truth has advanced past our watermark
  hasConflict(truthVersion: number): boolean {
    return truthVersion > this._versionWatermark && this._writes.size > 0
  }
}
...
export class ObjectStore {
  ...
  private _currentGcGeneration: number = 0    // NEW
  private _gcSweeps: number = 0               // NEW
  private _evictions: number = 0              // NEW

  constructor(config?: Partial<ObjectStoreConfig>) {
    this._config = { ...DEFAULT_CONFIG, ...config }
    this._links = new LinkRegistry()

    introspector.registerProbe('object-store', () => ({
      totalObjects: this._objects.size,
      totalLinks: this._links.totalLinks,
      totalWrites: this._totalWrites,
      totalReads: this._totalReads,
      gcSweeps: this._gcSweeps,
      evictions: this._evictions,
      optimisticLayers: this._optimisticLayers.size
    }))
  }
  ...
  write<T>(objectType: OntologyObjectType, primaryKey: string, value: T, ttlMs?: number): ObjectEntry<T> {
    ...
    const entry: ObjectEntry<T> = {
      ...
      gcGeneration: this._currentGcGeneration,
      __debug_origin: `write@${now}`
    }
    ...
    const change: ObjectStoreChange = {
      type: 'write', objectType, primaryKey, timestamp: now
    }
    ...
    introspector.trace('object-store', 'write', { objectType, primaryKey, version })
    return entry
  }
  ...
  read<T>(objectType: OntologyObjectType, primaryKey: string): T | null {
    ...
    entry.accessCount++   // LFU tracking
  ...
  delete(objectType: OntologyObjectType, primaryKey: string): boolean {
    ...
    const change: ObjectStoreChange = {
      type: 'delete', objectType, primaryKey, timestamp: Date.now()
    }
    ...
    introspector.trace('object-store', 'delete', { objectType, primaryKey })
    return true
  }
  ...
  countByType(objectType: OntologyObjectType): number {
    return this._typeIndex.get(objectType)?.size ?? 0
  }
  ...
  addLink(
    ...
    const change: ObjectStoreChange = {
      type: 'link-add', objectType: sourceType, primaryKey: sourceKey,
      linkType, targetType, targetKey, timestamp: Date.now()
    }
    ...
    introspector.trace('object-store', 'link-add', { sourceType, sourceKey, linkType, targetKey })
    return true
  }
  ...
  removeLink(
    ...
    const change: ObjectStoreChange = {
      type: 'link-remove', objectType: sourceType, primaryKey: sourceKey,
      linkType, targetType, targetKey, timestamp: Date.now()
    }
  ...
  writeOptimistic<T>(layerId: string, objectType: OntologyObjectType, primaryKey: string, value: T): void {
    ...
    const entry: ObjectEntry<T> = {
      ...
      expiresAt: null, status: 'optimistic',
      accessCount: 0, gcGeneration: 0
    }
    this._notifyObjectListeners(keyStr, entry as ObjectEntry, 'write')
  }
  ...
  commitOptimistic(layerId: string): string[] {
    ...
    for (const keyStr of affected) {
      ...
      if (optimistic) {
        ...
        this._ensureTypeIndex(optimistic.key.objectType).add(keyStr)
      }
    }

    this._optimisticLayers.delete(layerId)
    introspector.debug('object-store', 'optimistic-commit', { layerId, affected: affected.length })
    return affected
  }
  ...
  rollbackOptimistic(layerId: string): string[] {
    ...
    this._optimisticLayers.delete(layerId)
    introspector.debug('object-store', 'optimistic-rollback', { layerId, affected: affected.length })
    return affected
  }

  subscribe<T>(
    objectType: OntologyObjectType, primaryKey: string,
  ...
  subscribeType(objectType: OntologyObjectType, listener: TypeListener): () => void {
    ...
    return () => { this._typeListeners.get(objectType)?.delete(listener) }
  }
  ...
  // Changed: deduplicates changes by key before notification
  commitBatch(): void {
    ...
    // Deduplicate: keep last change per key
    const deduped = new Map<string, ObjectStoreChange>()
    for (const change of changes) {
      const dk = `${change.objectType}:${change.primaryKey}:${change.type}`
      deduped.set(dk, change)
    }
    const dedupedChanges = Array.from(deduped.values())

    const byType = new Map<OntologyObjectType, ObjectStoreChange[]>()
    for (const change of dedupedChanges) {
      if (!byType.has(change.objectType)) byType.set(change.objectType, [])
    ...
    this._notifyGlobalListeners(dedupedChanges)

    introspector.debug('object-store', 'batch-commit', {
      raw: changes.length, deduped: dedupedChanges.length
    })
  }
  ...
  // Changed: LFU eviction instead of oldest-updated
  private _evictIfNeeded(objectType: OntologyObjectType): void {
    ...
    if (minKey) {
      const key = stringToObjectKey(minKey)
      this.delete(key.objectType, key.primaryKey)
      this._evictions++
    }
  }

  // Changed: generational GC — objects survive N rounds before expiry check
  private _gc(): void {
    this._gcSweeps++
    this._currentGcGeneration++
    const now = Date.now()
    const expired: string[] = []

    for (const [keyStr, entry] of this._objects) {
      if (entry.expiresAt === null) continue
      // Only check entries that have survived enough GC generations
      const generations = this._currentGcGeneration - entry.gcGeneration
      if (generations < this._config.gcSurvivalGenerations) continue
    ...
    if (expired.length > 0) {
      introspector.debug('object-store', 'gc-sweep', {
        generation: this._currentGcGeneration,
  ...
  private _countSubscriptions(): number {
    let count = 0
    for (const [, listeners] of this._objectListeners) count += listeners.size
    for (const [, listeners] of this._typeListeners) count += listeners.size
...
export function debugPrintObjectStoreReport(store: ObjectStore): void {
  const stats = store.getStats()
  console.log('\n╔══════════════════════════════════════════════╗')
  console.log('║   NexusObjectStore — Report                  ║')
  console.log('╠══════════════════════════════════════════════╣')
  console.log(`║ Objects:      ${String(stats.totalObjects).padEnd(31)}║`)
  console.log(`║ Links:        ${String(stats.totalLinks).padEnd(31)}║`)
  console.log(`║ Writes:       ${String(stats.totalWrites).padEnd(31)}║`)
  console.log(`║ Reads:        ${String(stats.totalReads).padEnd(31)}║`)
  console.log(`║ Deletes:      ${String(stats.totalDeletes).padEnd(31)}║`)
  console.log(`║ GC sweeps:    ${String(stats.gcSweeps).padEnd(31)}║`)
  console.log(`║ Evictions:    ${String(stats.evictions).padEnd(31)}║`)
  console.log(`║ Subscriptions:${String(stats.totalSubscriptions).padEnd(31)}║`)
  console.log('╠══════════════════════════════════════════════╣')
  for (const [type, count] of Object.entries(stats.objectCountByType)) {
    console.log(`║  ${type.padEnd(14)} ${String(count).padEnd(28)}║`)
  }
  console.log('╚══════════════════════════════════════════════╝\n')
}
