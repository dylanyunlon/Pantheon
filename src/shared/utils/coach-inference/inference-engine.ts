import type { FeatureVector } from '../coach-capture/experiment-capture'
import type { CoachAdviceType, CoachAdvicePriority } from '../coach-engine'
...
export class CoachInferenceEngine {
  ...
  async loadModel(modelPath: string, metadata?: Partial<ModelMetadata>): Promise<boolean> {
    if (!this._sessionFactory) {
      console.warn('CoachInferenceEngine: no session factory set, cannot load ONNX model')
      return false
    }
    ...
    try {
      ...
      console.error('CoachInferenceEngine: failed to load model', error)
      this._errors++
      this._isReady = false
      return false
    } finally {
  ...
  switchBackend(backend: InferenceBackend): void {
    if (backend === 'onnx' && !this._session) {
      console.warn('CoachInferenceEngine: cannot switch to onnx without loaded model')
      return
    }
...
export function createInferenceEngine(config?: Partial<InferenceConfig>): CoachInferenceEngine {
  return new CoachInferenceEngine(config)
}
