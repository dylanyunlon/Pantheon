...
export class PantheonScheduler {
  ...
  enqueue(advices: PantheonAdvice[]): ScheduledAdvice[] {
    const now = Date.now()
...
export function createPantheonScheduler(config?: Partial<SchedulerConfig>): PantheonScheduler {
  return new PantheonScheduler(config)
}
