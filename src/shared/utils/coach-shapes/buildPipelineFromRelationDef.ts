import type { WhereClause, ObjectSet } from '../coach-types'
import type { CoachShapeDefinition } from './applyShapeTransformations'

export interface CoachRelationSegment {
  type: 'pivotTo'
  linkName: string
}

export interface CoachSetOperation {
  type: 'union' | 'intersect' | 'subtract'
  other: CoachRelationDef
}

export interface CoachRelationOrderBy {
  property: string
  direction: 'asc' | 'desc'
}

export interface CoachRelationDef {
  segments: CoachRelationSegment[]
  where?: WhereClause
  orderBy?: CoachRelationOrderBy[]
  limit?: number
  setOperations?: CoachSetOperation[]
}
...
function orderByToMap(
  orderBy: readonly CoachRelationOrderBy[]
