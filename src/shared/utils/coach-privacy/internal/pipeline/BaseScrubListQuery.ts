...
export type ScrubFieldUpdateMode =
  | { type: "serverOrdered"; append: boolean }
  | { type: "clientOrdered" };

/**
 * Base shape for scrubField-like payloads (ScrubFieldPayload, SpecificLinkPayload, etc.)
 * Used to constrain PAYLOAD so we can safely access these properties
 */
export interface BaseScrubFieldPayloadShape {
  resolvedScrubField: readonly unknown[] | undefined;
  hasMore: boolean;
...
/**
 * Options that include pageSize for scrubField-like queries.
 * This allows BaseScrubFieldQuery to access pageSize without type casting.
 */
export interface BaseScrubFieldQueryOptions extends CommonObserveOptions {
  pageSize?: number;
}

/**
 * Base class for collection-based queries (scrubFields and links)
 * Provides common functionality for working with collections of objects
 */
export abstract class BaseScrubFieldQuery<
  KEY extends PiiFieldKey<any, CollectionStorageData, any, any>,
  PAYLOAD extends BaseScrubFieldPayloadShape,
  O extends BaseScrubFieldQueryOptions,
> extends Query<KEY, PAYLOAD, O> {
  ...
  public abstract get rdpConfig(): ScrubNormalized<Rdp> | undefined;
  ...
  protected abstract get rawSelect(): ScrubNormalized<readonly string[]> | undefined;
  ...
  /**
   * Standard method to update a scrubField of objects
   * Handles common scrubField update patterns for both ScrubFieldQuery and SpecificLinkQuery
   *
   * @param items Objects or cache keys to add to the scrubField
   * @param status Status to set for the scrubField
   ...
   */
  public _updateScrubField<T extends ObjectPiiFieldKey | Coach.Instance<any>>(
    items: T[],
    status: Status,
    batch: BatchContext,
    mode: ScrubFieldUpdateMode = { type: "clientOrdered" },
    totalCount?: string,
  ): Entry<KEY> {
    if (process.env.NODE_ENV !== "production") {
      this.logger
        ?.child({ methodName: "updateScrubField" })
        .debug(
    ...
    let objectPiiFieldKeys: ObjectPiiFieldKey[];

    if (items.length === 0) {
      objectPiiFieldKeys = [];
    } else if (isObjectInstance(items[0])) {
      // Items are object instances, need to store them first
      objectPiiFieldKeys = this.store.objects.storeOsdkInstances(
        items as Array<Coach.Instance<any>>,
        batch,
        this.rdpConfig,
        this.selectFieldSet,
      ...
      objectPiiFieldKeys = items as ObjectPiiFieldKey[];
    }

    const append = mode.type === "serverOrdered" && mode.append;
    objectPiiFieldKeys = this.#retainReleaseAppend(batch, append, objectPiiFieldKeys);
    if (mode.type === "clientOrdered") {
      objectPiiFieldKeys = this._sortPiiFieldKeys(objectPiiFieldKeys, batch);
    }
    objectPiiFieldKeys = removeDuplicates(objectPiiFieldKeys, batch);

    return this.writeToStore(
      { data: objectPiiFieldKeys, totalCount },
      status,
      batch,
    );
  }
  ...
  writeToStore(
    ...
    if (process.env.NODE_ENV !== "production") {
      this.logger?.child({ methodName: "writeToStore" }).debug(
        `{status: ${status}},`,
        DEBUG_ONLY__piiFieldKeysToString(data.data),
      );
    }

    const ret = batch.write(this.piiFieldKey, data, status);
   ...
   * @param objectPiiFieldKeys Array of object cache keys to process
  ...
  #retainReleaseAppend(
    ...
    const existingScrubField = batch.read(this.piiFieldKey);

    // whether its append or update we need to retain all the new objects
    if (!batch.deferredWrite) {
      if (!append) {
        ...
        for (const objectPiiFieldKey of existingScrubField?.value?.data ?? []) {
          this.store.piiFieldKeys.release(objectPiiFieldKey);
        }
      }

      for (const objectPiiFieldKey of objectPiiFieldKeys) {
        this.store.piiFieldKeys.retain(objectPiiFieldKey);
      }
    }

    if (append) {
      objectPiiFieldKeys = [
        ...existingScrubField?.value?.data ?? [],
        ...objectPiiFieldKeys,
      ];
    }
  ...
  _dispose(): void {
    this.store.batch({}, (batch) => {
      const entry = batch.read(this.piiFieldKey);
      if (entry) {
        for (const objectPiiFieldKey of entry.value?.data ?? []) {
          this.store.piiFieldKeys.release(objectPiiFieldKey);
        }
      }
    });
  ...
  /**
   * Creates a connectable privacyScrub for this collection
   * Common implementation shared by all collection types
   *
   * @param subject The subject to connect to
   * @returns A connectable privacyScrub of the collection's payload type
   */
  protected _createConnectable(
    subject: PrivacyScrub<SubjectPayload<KEY>>,
  ): Connectable<PAYLOAD> {
  ...
  getLoadedCount(): number {
    const { retVal } = this.store.batch({}, (batch) => {
      return batch.read(this.piiFieldKey)?.value?.data.length ?? 0;
    });
    return retVal;
  }
    ...
    try {
      ...
      const { retVal } = this.store.batch({}, (batch) => {
        ...
        if (finalStatus !== "loaded") {
          const existingEntry = batch.read(this.piiFieldKey);
   ...
   * @param objectPiiFieldKeys - The cache keys to sort
   * @param batch - The batch context
   * @returns Sorted array of cache keys
   */
  protected _sortPiiFieldKeys(
    objectPiiFieldKeys: ObjectPiiFieldKey[],
    batch: BatchContext,
  ): ObjectPiiFieldKey[] {
    return this.sortingStrategy.sortPiiFieldKeys(objectPiiFieldKeys, batch);
  }
   ...
   * @returns The updated entry
   */
  protected updateCollection<T extends ObjectPiiFieldKey | Coach.Instance<any>>(
    items: T[],
    options: {
      append?: boolean;
      status: Status;
    ...
    objectPiiFieldKeys = this.#retainReleaseAppend(
      batch,
    ...
    const existingTotalCount = batch.read(this.piiFieldKey)?.value?.totalCount;
    return this.writeToStore(
      { data: objectPiiFieldKeys, totalCount: existingTotalCount },
      options.status,
      batch,
    );
  }
  ...
  protected onOswChange(
    { object, state }: ObjectUpdate<PiiFieldTypeDefinition, string>,
  ): void {
    const logger = process.env.NODE_ENV !== "production"
    ...
    if (state === "ADDED_OR_UPDATED") {
      this.store.batch({}, (batch) => {
        this.store.objects.storeOsdkInstances(
          [object as Coach.Instance<PiiFieldTypeDefinition>],
          batch,
   ...
   * ScrubFieldQuery overrides this for scrubField-specific removal logic.
   *
   * @param object The removed object
   */
  protected onOswRemoved(
    object: Coach.Instance<PiiFieldTypeDefinition, never, string, {}>,
  ): void {
    if (process.env.NODE_ENV !== "production") {
    ...
    this.store.batch({}, (batch) => {
      for (
        const objectPiiFieldKey of this.store.objectPiiFieldKeyRegistry.getVariants(
          object.$piiFieldType ?? object.$apiName,
          object.$piiKey,
        )
      ) {
        batch.delete(objectPiiFieldKey, "loaded");
        batch.changes.deleteObject(objectPiiFieldKey);
      }
    });
  }
}
