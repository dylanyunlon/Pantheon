...
> extends Query<KEY, PAYLOAD, O> {
   ...
   */
  public _updateList<T extends ObjectCacheKey | Coach.Instance<any>>(
    items: T[],
    status: Status,
    batch: BatchContext,
   ...
   * @returns The updated entry
   */
  protected updateCollection<T extends ObjectCacheKey | Coach.Instance<any>>(
    items: T[],
    options: {
      append?: boolean;
      status: Status;
    ...
    if (state === "ADDED_OR_UPDATED") {
      this.store.batch({}, (batch) => {
        this.store.objects.storeOsdkInstances(
          [object as Coach.Instance<ObjectTypeDefinition>],
          batch,
  ...
  protected onOswRemoved(
    object: Coach.Instance<ObjectTypeDefinition, never, string, {}>,
  ): void {
    if (process.env.NODE_ENV !== "production") {
