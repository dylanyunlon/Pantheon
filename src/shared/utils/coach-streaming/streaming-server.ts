...
export class CoachStreamServer {
  ...
  async start(sessionId?: string): Promise<boolean> {
    ...
    try {
      ...
      console.error('CoachStreamServer: failed to start', error)
      return false
    }
  }
  ...
  broadcastAdvices(advices: CoachAdvice[], gamePhase: GamePhase): void {
    this._enqueue({
...
export function createStreamServer(config?: Partial<StreamServerConfig>): CoachStreamServer {
  return new CoachStreamServer(config)
}
