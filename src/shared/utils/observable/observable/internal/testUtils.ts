...
import type { OrderBy, Status } from "../ObservableClient/common";
import type { Unsubscribable } from "../Unsubscribable";
import type { Entry } from "./Layer";
import type { ListQueryOptions } from "./list/ListQueryOptions";
...
export function createClientMockHelper(): MockClientHelper {
  ...
  function mockFetchPageOnce<
    X extends PageResult<
      Coach.Instance<ObjectTypeDefinition>
    >,
  >(): DeferredPromise<X> {
    const d = pDefer<X>();
  ...
  function mockFetchOneOnce<
    X extends Partial<PantheonBase<any>>,
    ...
    client.mockReturnValueOnce(
      {
        applyAction: async (_args): Promise<ActionEditResponse> => {
          ...
          return {
            ...
            editedObjectTypes: (x as any).editedObjectTypes ?? [],
...
export function expectSingleLinkCallAndClear<T extends ObjectTypeDefinition>(
  ...
  resolvedList: ObjectHolder[] | Coach.Instance<T>[] | undefined,
...
export function updateList<T extends ObjectOrInterfaceDefinition>(
  ...
  objects: ObjectHolder[] | Coach.Instance<T>[],
