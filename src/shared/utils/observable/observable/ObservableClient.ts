...
import { additionalContext, type Client } from "../engine";
...
export interface ObserveObjectCallbackArgs<
  T extends ObjectOrInterfaceDefinition,
> {
  object: Coach.Instance<T, "$allBaseProperties"> | undefined;
  isOptimistic: boolean;
  status: Status;
  lastUpdated: number;
}
...
> {
  resolvedList:
    | Array<Coach.Instance<T, "$allBaseProperties", PropertyKeys<T>, RDPs>>
    | undefined;
  isOptimistic: boolean;
  lastUpdated: number;
...
> {
  resolvedList:
    | Array<
      Coach.Instance<T, "$allBaseProperties", PropertyKeys<T>, RDPs>
    >
    | undefined;
  isOptimistic: boolean;
  lastUpdated: number;
...
export interface ObservableClient extends ObserveLinks {
  ...
  invalidateObjects(
    objects:
      ...
      | ReadonlyArray<Coach.Instance<ObjectTypeDefinition>>,
  ): Promise<void>;
