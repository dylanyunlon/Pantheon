...
export class PantheonCacheLayer<T> {
  private _parent: PantheonCacheLayer<T> | undefined
  private _cache = new Map<string, CacheEntry<T>>()
  private _layerId: string | undefined

  constructor(parent: PantheonCacheLayer<T> | undefined, layerId: string | undefined) {
    this._parent = parent
    this._layerId = layerId
  }

  get parentLayer(): PantheonCacheLayer<T> | undefined {
    return this._parent
  }
  ...
  get layerId(): string | undefined {
    ...
    addLayer(layerId: string): PantheonCacheLayer<T> {
    return new PantheonCacheLayer<T>(this, layerId)
  }

    removeLayer(layerId: string): PantheonCacheLayer<T> {
...
export class PantheonCacheLayers<T> {
  private _truthLayer: PantheonCacheLayer<T>
  private _topLayer: PantheonCacheLayer<T>

  constructor() {
    this._truthLayer = new PantheonCacheLayer<T>(undefined, undefined)
    this._topLayer = this._truthLayer
  }

  get truth(): PantheonCacheLayer<T> {
    return this._truthLayer
  }

  get top(): PantheonCacheLayer<T> {
    ...
    let current: PantheonCacheLayer<T> | undefined = this._topLayer
