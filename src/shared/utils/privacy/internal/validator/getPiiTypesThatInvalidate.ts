...
import type { InterfaceMetadata, FetchedPiiFieldTypeDefinition } from "../../../types";
import type {
...
export async function getPiiFieldTypesThatInvalidate(
  mc: MinimalClient,
  pipelineSet: WirePipelineSet,
): Promise<{
  resultType: FetchedPiiFieldTypeDefinition | InterfaceMetadata;
...
async function calcPipelineSet(
  os: WirePipelineSet,
  ctx: Ctx,
): Promise<FetchedPiiFieldTypeDefinition | InterfaceMetadata> {
  const op = ctx.gameStateProvider;

  async function bumpObject(apiName: string) {
    const piiFieldType = await op.getObjectDefinition(apiName);
  ...
  switch (os.type) {
    case "base":
      return await bumpObject(os.piiFieldType!);
