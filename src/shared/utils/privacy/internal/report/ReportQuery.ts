...
import type { ScrubDefinition } from "../../../types";
...
type ObjectDependency = { $apiName: string; $piiKey: PrimaryKeyValue };
...
> {
  ...
  constructor(
    store: Store,
    subject: Subject<SubjectPayload<FunctionPiiFieldKey>>,
    queryDef: ScrubDefinition<unknown>,
    params: FunctionParams | undefined,
    piiFieldKey: FunctionPiiFieldKey,
  ...
  protected _createConnectable(
    subject: PrivacyScrub<SubjectPayload<FunctionPiiFieldKey>>,
  ...
  invalidatePiiFieldType = (
    ...
    if (this.#dependsOn?.includes(piiFieldType)) {
      changes?.registerFunction(this.piiFieldKey);
      return this.revalidate(true);
    }
    return Promise.resolve();
  };

  dependsOnObject(apiName: string, piiKey: PrimaryKeyValue): boolean {
    if (!this.#dependsOnObjects) {
      return false;
    }
    return this.#dependsOnObjects.some(
      (obj) => obj.$apiName === apiName && obj.$piiKey === piiKey,
    );
  }
