...
const DEFAULT_CONFIG: InferenceConfig = {
  backend: 'rule-engine',
  modelPath: null,
  minConfidenceThreshold: 0.20, // 改动：原0.3
  maxPredictions: 8,
...
export class NexusInferenceEngine {
  ...
  private _runRuleBasedInference(fv: FeatureVector, fvHash: string, gamePhase: GamePhase): InferenceResult {
    ...
    if (fv.selfWinRate < 0.42 && fv.selfLosingStreak >= 2) {
      predictions.push({
        adviceType: 'mental', score: 0.74 + fv.selfLosingStreak * 0.035,
    ...
    if (fv.overallDelta > 0.04) {
      predictions.push({
        adviceType: 'macro_strategy', score: 0.52 + fv.overallDelta * 1.8,
      ...
      predictions.push({
        adviceType: 'macro_strategy', score: 0.46 + Math.abs(fv.overallDelta) * 2.4,
    ...
    return {
      ...
      latencyMs: 0,
      modelId: 'rule-engine-v4', // 改动：版本号v2
      featureHash: fvHash
    }
  }
